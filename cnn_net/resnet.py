...
def identity_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = Conv2D(nb_filter2, kernel_size, 1, padding='same',
...
def conv_block(input_tensor, kernel_size, filters, stage, block,
               strides=2, use_bias=False, train_bn=True):
    nb_filter1, nb_filter2, nb_filter3 = filters
    conv_name_base = 'res' + str(stage) + block + '_branch'
    bn_name_base = 'bn' + str(stage) + block + '_branch'

    x = Conv2D(nb_filter1, 1, strides=strides,
    ...
    x = Conv2D(nb_filter3, 1, name=conv_name_base + '2c', use_bias=use_bias)(x)
    x = BatchNormalization(name=bn_name_base + '2c')(x, training=train_bn)

    shortcut = Conv2D(nb_filter3, 1, strides=strides,
...
def get_resnet(input_image, stage5=False, train_bn=True):
    # Stage 1
    x = ZeroPadding2D(padding=(3, 3))(input_image)  # 0填充
    # Height/2,Width/2,64
    x = Conv2D(64, 7, strides=2, name='conv1', use_bias=False)(x)
    x = BatchNormalization(name='bn_conv1')(x, training=train_bn)
    x = Activation('relu')(x)
    # Height/4,Width/4,64
    C1 = x = MaxPooling2D(pool_size=3, strides=2)(x)
    # Stage 2
    ...
    # Stage 4
    x = conv_block(x, 3, [64, 64, 1024], stage=4, block='a', train_bn=train_bn)
    block_count = 22
    for i in range(block_count):
        x = identity_block(x, 3, [64, 64, 1024], stage=4, block=chr(98 + i), train_bn=train_bn)
    # Height/16,Width/16,1024
    C4 = x
    # Stage 5
    if stage5:
        x = conv_block(x, 3, [32, 32, 2048], stage=5, block='a', train_bn=train_bn)
        x = identity_block(x, 3, [32, 32, 2048], stage=5, block='b', train_bn=train_bn)
        # Height/32,Width/32,2048
        C5 = x = identity_block(x, 3, [32, 32, 2048], stage=5, block='c', train_bn=train_bn)
    else:
        C5 = None
